...
The update lease is wall-clock crash/replay accounting only; it does not fence command side effects. Until OS-held single-instance fencing is added in the next stage, operate one host with bounded clock adjustments and configure the supervisor to stop the old process fully before starting a replacement. Do not intentionally overlap application instances: a forward clock jump or overlap can let a new owner reclaim the journal while old domain work is still running.
