...
Numeric config values are strict in production: malformed integers fail startup instead of falling back to defaults. Keep `OBS_PORT` in `1..65535`, set `MAX_VIDEO_SIZE_MB` and `MAX_QUEUE_LENGTH` above `0`, use non-negative values for `MAX_VIDEO_DURATION_SECONDS`, `MIN_FREE_DISK_MB`, and `RETENTION_MAX_FILES`, and keep `RETENTION_DAYS` in `0..106751`. A value of `0` remains valid where it means disabled. `MIN_FREE_DISK_MB` defaults to `512`; setting it to `0` disables only the additional reserve, not actual-size filesystem admission or `MAX_VIDEO_SIZE_MB`. `RETENTION_DELETE_LOCAL_FILES` must be boolean and defaults to `false`.
...
Upload admission is two-stage. Before calling Local Bot API `getFile`, the app requires a positive Telegram-declared size. Queue mode rejects a full queue; library mode rejects an invalid filename or an existing destination. Queue mode requires `declared size + MIN_FREE_DISK_MB` on the configured `TELEGRAM_BOT_API_DIR` filesystem. Library mode detects whether that cache and its selected destination share a filesystem: a shared filesystem requires `2 × declared size + MIN_FREE_DISK_MB` because the cached source and staging copy coexist, while distinct filesystems each require `declared size + MIN_FREE_DISK_MB`. The library directories are created during startup before Telegram handling begins, and inability to inspect either filesystem fails closed. These deterministic rejections therefore do not ask Local Bot API Server to populate its cache.

After `getFile`, the app still validates the returned absolute path and size and rechecks admission from the actual on-disk file. This closes metadata differences and races: queue capacity, destination collisions, and actual filesystem headroom remain authoritative at enqueue/import time. If declared metadata passes but differs from the local file, media/content validation fails, or application/filesystem state changes concurrently, the later check can still reject after Local Bot API Server cached the file. The app does not delete that Telegram-owned cache file because it may be referenced elsewhere.

Persistent-state ceilings are fixed rather than configurable. Each `LOOP_MEDIA_DIR` and `MUSIC_MEDIA_DIR` may contain at most 10,000 top-level entries, including the app-owned staging directory, and the SQLite `videos` table may contain at most 10,000 total rows across all statuses. Preflight rejects a full destination or table before `getFile`; library import and the atomic video-row insert recheck authoritatively. Remove obsolete library entries manually, or allow the configured terminal-history retention policy to free video rows. The ceiling itself never triggers automatic deletion. If an external writer pushes a library directory over capacity, scans retain the last-known-good snapshot and report the capacity error; an over-capacity startup remains empty/degraded instead of forcing a restart loop.
