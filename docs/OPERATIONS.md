...
4. Configure Telegram group access:
   ...
   - make sure library managers are Telegram group admins.
...
The service is library-only. Store loop videos in `LOOP_MEDIA_DIR` and music files in `MUSIC_MEDIA_DIR`; defaults are `./data/media/loops` and `./data/media/music`.
...
Numeric config values are strict in production: malformed integers fail startup instead of falling back to defaults. Keep `OBS_PORT` in `1..65535`, set `MAX_VIDEO_SIZE_MB` above `0`, and use non-negative values for `MAX_VIDEO_DURATION_SECONDS` and `MIN_FREE_DISK_MB`. A value of `0` remains valid where it means disabled. `MIN_FREE_DISK_MB` defaults to `512`; setting it to `0` disables only the additional reserve, not actual-size filesystem admission or `MAX_VIDEO_SIZE_MB`.

Env schema v7 removes the runtime mode switch and all queue-only settings.
Fresh configurations default to `DATA_DIR/state.db`. For schema v6 or older,
migration preserves an explicit `DATABASE_PATH`; when the key is missing or
empty, it first materializes the historical `DATA_DIR/queue.db` path and then
advances the schema. This prevents an upgrade from silently opening a new
empty database. The loader accepts a deprecated `PLAYER_MODE=library` entry
only long enough for an unmigrated deployment to reach the migration path;
`./run.sh migrate-env` removes it and the other obsolete queue keys.
`PLAYER_MODE=queue` fails closed before any migration write with an actionable
error; first create and configure both library OBS sources and populate the
media directories, then change it to `library` and run migration. Migration
never plays, updates, or deletes historical `videos` rows or Telegram-owned
cache files.
...
Inside `tg-obs-bot`, Telegram polling, OBS reconnect, OBS events, periodic maintenance, and the library scheduler are required workers. The coordinator does not run maintenance inline, so blocked cleanup cannot conceal another worker's failure. An unexpected worker exit stops the process. Telegram update handlers have a five-minute deadline plus a five-second cancellation grace; a non-cooperative handler causes exit status `70` so it cannot leave polling permanently stuck. Internal worker shutdown is bounded to ten seconds: five seconds for handler cancellation, up to four seconds for independent journal finalization, and a scheduling cushion. Normal `TERM` remains graceful when every worker cooperates.

`./run.sh up` enables the local worker watchdog for every app generation. It creates an effective-UID-owned private directory in the fixed OS temporary base (`/private/tmp` on macOS or `/tmp` on Linux), installs a mode-`0600` FIFO as descriptor 3, and sets `TG_OBS_LIVENESS_FD3=1` only for that supervised app. The strict local reader accepts only exact `TGOBS1` records with increasing frame generations and nondecreasing worker sequences. Equal sequences must retain their phase and do not renew a lease. The channel carries only counters and phases—never provider credentials, Telegram/OBS payloads, or a user-configurable path. `./run.sh app` explicitly unsets the marker and does not require the reader.

The fixed startup allowance is 360 seconds. After the first valid frame, each worker has its own no-progress limit: 60 seconds in phases `00`-`07`, 150 seconds in phase `08`, and 310 seconds in phases `09`-`10`. A malformed or reordered frame, sequence decrease, phase change without progress, lost reader/channel, failed private-state install, startup timeout, or stale worker fails only the current app generation. The log reports a fixed `reason`, plus worker, phase, age, and reader status where applicable; the existing process-group drain and exponential backoff then replace the app without restarting the Telegram Local Bot API sibling.

These files are local implementation state, not a health API or service-manager interface. Do not monitor or write the FIFO directly. On `HUP`, `INT`, or `TERM`, the app supervisor stops and reaps its reader before removing the generation FIFO/state; the root supervisor removes the enclosing private directory after both service supervisors have stopped.

Transient recurring failures are intentionally rate-limited inside the Go backend. Telegram polling retries grow from three seconds to one minute with bounded jitter; a Telegram `retry_after` response can extend that wait up to fifteen minutes. OBS connect/probe/playback-recovery retries grow from five seconds to one minute. Library reconciliation grows to five minutes but is capped at the next time-period boundary so an old loop cannot survive a transition solely because of retry sleep; ten-minute maintenance grows to one hour. A successful attempt restores the normal cadence. Time spent disconnected or skipped because another OBS recovery owns the operation does not erase or inflate the attempted-failure count.

For one continuous failure burst, expect a warning on the first failure and every eighth failure, with `consecutive_failures` and `suppressed` fields. Retrying loops then emit one recovery entry. OBS event processing never sleeps and does not emit recovery entries; its repeated decode/overflow/advance warnings reset silently only after eight consecutive healthy events so alternating success/failure traffic cannot flood logs. Error fields on these recurring paths are secret-redacted and capped at 512 valid UTF-8 bytes. Reconnect library-scan warnings are aggregated or sampled; consult `/status` for the retained latest error when an individual occurrence was suppressed.
...
- `/library [page]`: show paginated loop/music library counts and useful asset IDs.
...
- `/theme <theme|random>`: set today's theme override or return to random.
  A theme must fit both the 240-rune and 240-byte UTF-8 bounds and may not
  contain `_`, `/`, or `\`.
...
Imported media is copied into `LOOP_MEDIA_DIR` or `MUSIC_MEDIA_DIR`. Remove obsolete library assets manually during a maintenance window, then run `/scan`. The service never deletes production library assets automatically.

`/status` probes and reports `LOOP_MEDIA_DIR`, `MUSIC_MEDIA_DIR`, and `TELEGRAM_BOT_API_DIR` separately, so paths on different filesystems never reuse another path's free/total values. It also reports playable counts, scan rejections, runtime quarantines, overrides, preview state, and the retained last error. If paths are relative, inspect them from the repository root, or use the absolute resolved paths shown by `./run.sh doctor`.

Upload admission is two-stage. Before calling Local Bot API `getFile`, the app requires a positive Telegram-declared size, a valid library filename, an available destination name, directory capacity, and filesystem headroom. When the Telegram cache and selected library destination share a filesystem, admission requires `2 × declared size + MIN_FREE_DISK_MB` because the cached source and staging copy coexist. On distinct filesystems, each requires `declared size + MIN_FREE_DISK_MB`. The library directories are created during startup before Telegram handling begins, and inability to inspect either filesystem fails closed. These deterministic rejections therefore do not ask Local Bot API Server to populate its cache.

After `getFile`, the app still validates the returned absolute path, source identity, actual size, media stream type, destination collision, capacity, and filesystem headroom. If declared metadata passes but differs from the local file, media/content validation fails, or application/filesystem state changes concurrently, the later check can still reject after Local Bot API Server cached the file. The app does not delete that Telegram-owned cache file because it may be referenced elsewhere.

Persistent-state ceilings are fixed rather than configurable. Each `LOOP_MEDIA_DIR` and `MUSIC_MEDIA_DIR` may contain at most 10,000 top-level entries, including the app-owned staging directory. Preflight and publication recheck the destination authoritatively. The ceiling never triggers automatic deletion. If an external writer pushes a library directory over capacity, scans retain the last-known-good snapshot and report the capacity error; an over-capacity startup remains empty/degraded instead of forcing a restart loop.

Library imports copy into the app-owned `.tg-obs-bot-staging` subdirectory on the destination filesystem, flush and close the staging file, validate media there, then publish it atomically without replacing an existing destination. The staging and destination directories are both synchronized after publication. Collision, cancellation, validation failure, write failure, and `ENOSPC` remove the staging file without exposing an invalid final asset. Startup and ten-minute maintenance inspect at most 256 entries per staging directory and remove only matching regular staging files older than six hours; later passes converge on larger backlogs while unrelated, non-regular, and newer files remain untouched.
...
Operator-added files are not trusted merely because their names and
extensions match. `/scan` applies the same size, duration, and required-stream
validation as uploads. Invalid files stay out of the playable snapshot and
appear as scan warnings. If OBS explicitly reports a media error for an active
asset, the scheduler quarantines it and tries another matching candidate in a
bounded pass; replacing or modifying that file makes it eligible for
validation again.
